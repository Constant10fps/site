<a
    class="flex flex-col items-center gap-3 p-3 bg-left-top bg-slate-700 rounded-2xl hover:bg-slate-600 active:scale-95 transition-all"
    href="https://github.com/Constant10fps/shooter"
>
    <img
        src="/pythonlogo.png"
        alt="logo"
        class="rounded-2xl aspect-square size-36"
    />
    <div class="flex flex-col items-start">
        <div class="text-xl font-bold">Python Project Example</div>
        <div class="text-base font-medium text-left">
            Lorem ipsum dolor sit amet, consectetur adipiscing elit. Cras nibh
            arcu, sollicitudin eget neque et, viverra maximus ex.
        </div>
    </div>
</a>
